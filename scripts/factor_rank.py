...
from ashare_gauntlet.factor_model import composite, factor_percentile, to_decile
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f"=== 横截面因子排序(质量×价值·行业中性·零magic number, as_of={as_of}, {len(df)}只)→ {OUT_DIR}/{as_of}_factor.json ===")
    print("分位=行业内百分位(0-100,越高越好);ACC=低应计(已转好=高分位);D=十分位(D10最优)")
    print(f"{'#':>2} {'D':>2} {'档':>2} {'票':<9}{'行业':<7}{'EP':>3}{'BP':>3}{'ROE':>4}{'GP':>3}{'ACC':>4}{'PE':>5}{'ROE%':>5}")
    ...
    for i, (code, r) in enumerate(df.head(a.top).iterrows(), 1):
        print(f"{i:>2} {(int(r['decile']) if pd.notna(r['decile']) else 0):>2} {r['tier']:>2} "
              ...
              f"{pc(r['f_EP']):>3}{pc(r['f_BP']):>3}{pc(r['f_ROE']):>4}{pc(r['f_GPOA']):>3}{pc(r['f_ACC']):>4}"
              f"{nn(r['pe']):>5}{nn(r['roe']):>5}")
