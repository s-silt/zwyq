"""A 层(IO/CLI):读 data/cards/<as_of>.json,调纯渲染 render_md,写 data/panels/<as_of>.md。

纯渲染逻辑在 ashare_gauntlet/render_md.py(无 IO);此处只做读 json / 选最新 as_of /
写产物文件,贴合 scripts/cards.py 既有 pattern。

Usage:
  PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m scripts.panel            # 取 data/cards 下最新
  PYTHONIOENCODING=utf-8 .venv/Scripts/python.exe -m scripts.panel --as-of 20260618
"""
...
def main(as_of: str | None = None) -> None:
    ...
    out_path = f"{PANELS_DIR}/{resolved}.md"
    with open(out_path, "w", encoding="utf-8") as fh:
        fh.write(md)
    print(f"诚实面板 -> {out_path}（{len(records)} 只，{len(md)} 字符）")
