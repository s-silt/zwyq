...
口径边界(别把本表当生产净超额的监控器):所有行都是"快照日→同一个今日"的累计收益,终点
相同、窗口层层嵌套,相邻行近乎同一笔观测——不是独立月度观测,不可跨行读趋势,老快照行
数值更高多半只因窗口更长;成本整段只记一次买卖,而生产纪律(月度审视+C2)同一段时间要
按换手 τ 逐期付费,窗口越长本列的成本负担越趋近于零;基准也不同,成本后列对沪深300(配
置基准),§10 的"超额净"对宇宙等权(且本表左侧"超额"列虽对宇宙等权却不扣成本)。三处口
径都不同 → 本表与 docs/methodology.md §10 的组合级读数不可直接比大小。
...
def main() -> None:
    ...
    # 读法护栏写进输出而不只写进 docstring:实际读表的人看的是终端,曾据此把本表当成
    # "逐月净超额序列"读并挂上复审触发器(§10 判读2/5 旧措辞)
    print("(读法:每行都是「该快照日 → 同一个今日」的累计收益,所有行共用同一终点、窗口"
          "层层嵌套——相邻行近乎同一笔观测,不是独立月度读数,不可跨行读趋势;老快照行"
          "数值更高多半只因窗口更长)")
    print("(与 §10 生产读数不可直接比大小:本列对沪深300、整段只记一次买卖,§10 的"
          "「超额净」对宇宙等权、按换手 τ 逐期重复付费;本表左侧「超额」列对宇宙等权"
          "但不扣成本)")
