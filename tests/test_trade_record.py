...
def test_sell_holdings_replace_failure_prints_recovery(tmp_path, monkeypatch, capsys):
    hp, jp, cache = _setup(tmp_path)
    ...
    monkeypatch.setattr(tr.os, "replace", failing_replace)
    with pytest.raises(OSError):
        _sell(hp, jp, cache)
    assert "恢复指引" in capsys.readouterr().out
